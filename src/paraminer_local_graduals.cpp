...
int compute_graank_support(const BoolMatrix &BM, int n){
    float tmp = BM.graank();  //((n*(n-1))/2);
    //int res = round(tmp*100);
    //cout<<res<<endl;
    return tmp;
}
...
for(int i = 0; i < s.size()-1; i++){
  ...
  vector<int> path_length(nb_max_tids.first, 0);
    ...
    /*GRAANK HERE*/
    int sup2 = compute_graank_support(bm,nb_max_tids.first);
    return sup2>=threshold?sup2:0;
